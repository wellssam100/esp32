...
static bool i2c_slave_receive_cb(i2c_slave_dev_handle_t i2c_slave, const i2c_slave_rx_done_event_data_t *evt_data, void *arg){
    //uint8_t data = *evt_data->buffer;
    ESP_DRAM_LOGI("Example", "HELLOOO_____");
    //printf("Received Data %u", data);
    return false;
}
...
void waiting(void * pvParameters){
    while(true){
        printf("Wait\n");
...
i2c_slave_dev_handle_t setup_node_bus(){
    ...
    ESP_ERROR_CHECK(i2c_slave_register_event_callbacks(dev_handle, &cbs, NULL));
    //printf("I2C slave initialized with address 0x%X", 0x58);
    return dev_handle;
}
...
void app_main(void){
    ...
    xTaskCreate(&waiting, "Waiting", 2048, NULL, 1, NULL);
